...
Matching problems admit several optimality criteria, as surveyed by Cseh~(2017).
...
\emph{Popular matching}, introduced by Gärdenfors~(1975), offers an alternative optimality notion suited to one-sided models, satisfying a trade-off between the number of agents matched and the respect of preferences.
A matching is popular if no other matching is preferred by a more agents (see the formal definition in the Preliminaries). 
Every stable matching is popular, and every popular matching is Pareto-optimal, but neither converse holds in general (Cseh,~2017).
...
Cseh~(2017) illustrates this with an example of the voting paradox of Condorcet (e.g. figure 6.1 in the paper).
...
Abraham et al.~(2007) provide a algorithmic treatment of the one-sided model. 
...
The algorithm of Abraham et al.~(2007) relies on two key notions: the \emph{fpost} of an agent, defined as her most preferred object, and her \emph{spost}, defined as her second-most preferred object among those not ranked first by any agent. 
...
This observation is central to the algorithmic challenges that arise in our setting. 
In this sense, the characterization of Abraham et al.~(2007) serves as a structural baseline for redesigning algorithms under incomplete information.

A related notion is \emph{rank-maximal matching}, introduced by Irving~(2003).
A rank-maximal matching maximizes the number of agents assigned to their most preferred object.
Unlike popular matchings, a rank-maximal matching always exists and can be found in polynomial time (Irving et al.,~2006). 
This guarantee makes rank-maximal matching a useful baseline when popular matchings do not exist.

Furthermore, the extension of Abraham et al.~(2007) to preference lists with ties provides a useful technical analogy for our setting.
...
%     There exists a perfect matching if and only if for every subset of agents, the number of allocated objects to that subset is greater than or equal to the number of agents in that subset. 
%     Formally, for every subset $S \subseteq \agents$, we have $|S| \leq |\match(S)|$
% \end{theorem}
...
% The fpost is the most preferred object of an agent, and the spost is the second choice of an agent among those that was not ranked first by any agent.
...
% Abraham et al.) (2007) worked on the one-sided model.

% Abraham et al. (2007) have provided a polynomial algorithm for deciding whether there exists a popular matching in the general case, where preferences can be ties.
% Abraham et al. (2007): There exists an O(n+m) algorithm that output a largest cardinality popular matching or a proof for its nonexistence.
...
% There exist algorithms for comparing matchings with respect to popularity. Each agent votes for the matching in which she is better off, and the matching with the most votes wins.

% A stable matching is not guaranteed to exist and there exists a polynomial algorithm
% to find it for non bipartite model. Idea: we can prove stability to deduce popularity, but it can be more difficult if the stability is a hard property.

% For capacitated popular matching problem there exists an linear algorithm to determine the existence of a popular matching and find a largest popular matching.

% Search for a popular matching:
% • Construct the one-sided graph, where each agent is adjacent to their first and second
% choice objects only.
% • Check for a matching that matches all agents and all first choice objects.
% • If some agents cannot have their first choice, we try to give them their second choice
% by reordering the attributions to have the lower number of agents with their last
% resort object.
