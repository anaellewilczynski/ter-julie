...
\title{Possible and Necessary Popular Matchings under Partial Preferences}
...
As a first step, we consider a simplified setting where agents have strict linear order preferences and for which there exists only perfect matchings.
...
As discussed in the \citep[Chapter 12]{brandt2016handbook}, fairness criteria such as envy-freeness or maximin may conflict with efficiency requirements such as Pareto efficiency or utilitarian optimality, which respectively aim at maximizing the overall welfare and avoiding loss of resources. 
...
\cite{aziz2013popular} study the relationship between popularity and envy-freeness, and show that the two criteria are incompatible in general when the number of agents exceeds three. 
...
Each applicant $a_i$ expresses their incomplete preferences by giving a strict partial order $P_i$ over the set of houses $\objects$. 
...
In this setting, we assume that the preferences are transitive.
...
\begin{itemize}
    \item Can structural properties of popular matchings, such as the characterization of \cite{abraham2007popular}, be extended to partial preferences to yield tractable algorithms?
