...
\begin{itemize}
    \item $\sigma(j) P_i \sigma(i)$ and for $o \in O$ such that $(\sigma(j),o) \notin P$ add $(\sigma(j),o) in \succ_j$
...
Hence:
\begin{enumerate}
    \item Because there are as many agents as houses, for all $i \in N$, $\mu(i)$ is a $fpost$ of any agent $j$ such that $fpost(j) \succ_i \mu(i)$.
